    ...
    <AnimatePresence>
      {show && (
        <motion.button
          ...
          className="fixed bottom-6 right-6 z-40 w-11 h-11 rounded-full bg-gold-500 hover:bg-gold-400 text-navy-900 flex items-center justify-center shadow-gold transition-colors duration-300 ease-premium"
        >
          <ArrowUp size={18} />
        </motion.button>
      )}
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {FOOTER_LINKS.quick.map((l) => (
                <li key={l.label}>
                  <a href={l.href} className="text-sm text-white/60 hover:text-gold-400 transition-colors duration-200">
                    {l.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {COURSES.slice(0, 6).map((c) => (
                <li key={c.title}>
                  <a href="#courses" className="text-sm text-white/60 hover:text-gold-400 transition-colors duration-200">
                    {c.title}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              <li className="flex items-center gap-2 text-sm text-white/60">
                <Phone size={14} className="text-gold-500" /> {SITE.phoneMale}
              </li>
              <li className="flex items-center gap-2 text-sm text-white/60">
                <Phone size={14} className="text-gold-500" /> {SITE.phoneFemale}
              </li>
              <li className="flex items-center gap-2 text-sm text-white/60">
                <Mail size={14} className="text-gold-500" /> {SITE.email}
              </li>
            </ul>
          </div>
        </div>
